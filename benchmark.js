...
const keys = Array.from({ length: 100000 }, (_, i) => `cache-v${i}`);
...
// Warmup
for(let i=0; i<10; i++) { original(); optimizedLoop(); optimizedReduce(); }

const origDuration = runBenchmark('Original (filter + map)', original);
const optLoopDuration = runBenchmark('Optimized (for loop)', optimizedLoop);
const optReduceDuration = runBenchmark('Optimized (reduce)', optimizedReduce);

console.log(`\nImprovement (for loop): ${((origDuration - optLoopDuration) / origDuration * 100).toFixed(2)}% faster`);
